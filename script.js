...
// camera-besturing (rotatie + zoom rond een middelpunt)
let cameraTarget = new THREE.Vector3(0, 0, 0);
let cameraDistance = 590;
let cameraTheta = 1.2;   // horizontale hoek
let cameraPhi = 0.7;     // verticale hoek (kleiner = meer van bovenaf)
...
function setupScene() {
  ...
  // --- Muisbesturing: slepen om te draaien, scrollen om te zoomen ---
  ...
  renderer.domElement.addEventListener("wheel", (e) => {
    e.preventDefault();
    cameraDistance = clamp(cameraDistance * (1 + e.deltaY * 0.0012), 15, 3000);
    updateCameraPosition();
  }, { passive: false });

  // --- Touch-besturing (mobiel): 1 vinger slepen, 2 vingers zoomen ---
  ...
  renderer.domElement.addEventListener("touchmove", (e) => {
    ...
    if (e.touches.length === 1 && isDragging) {
      ...
      if (lastTouchDist) {
        cameraDistance = clamp(cameraDistance * (1 + (lastTouchDist - d) * 0.002), 15, 3000);
        updateCameraPosition();
      }
      lastTouchDist = d;
    }
